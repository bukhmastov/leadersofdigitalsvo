...
# This script brings up a Hyperledger Fabric network for amigosing smart contracts
# and applications. The amigos network consists of two organizations with one
...
# Versions of fabric known not to work with the amigos network
...
function checkPrereqs() {
  ...
  for UNSUPPORTED_VERSION in $NONWORKING_VERSIONS; do
    ...
    if [ $? -eq 0 ]; then
      fatalln "Local Fabric binary version of $LOCAL_VERSION does not match the versions supported by the amigos network."
    fi

    infoln "$DOCKER_IMAGE_VERSION" | grep -q $UNSUPPORTED_VERSION
    if [ $? -eq 0 ]; then
      fatalln "Fabric Docker image version of $DOCKER_IMAGE_VERSION does not match the versions supported by the amigos network."
    fi
  done
...
# meant for development and amigosing that can quickly create the certificates and keys
...
# certificates, and MSP folders that are needed to create the amigos network in the
