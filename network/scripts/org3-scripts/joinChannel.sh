...
set -x
peer channel fetch 0 $BLOCKFILE -o orderer.leadersofdigitalsvo.ru:7050 --ordererTLSHostnameOverride orderer.leadersofdigitalsvo.ru -c $CHANNEL_NAME --tls --cafile "$ORDERER_CA" >&log.txt
res=$?
{ set +x; } 2>/dev/null
cat log.txt
