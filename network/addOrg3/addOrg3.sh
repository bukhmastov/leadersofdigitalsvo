...
# This script extends the Hyperledger Fabric amigos network by adding
...
function printHelp () {
  ...
  echo "      - 'up' - add org3 to the sample network. You need to bring up the amigos network and create a channel first."
  echo "      - 'down' - bring down the amigos network and org3 nodes"
  echo "      - 'generate' - generate required certificates and org definition"
  echo "    -c <channel name> - amigos network channel name (defaults to \"mychannel\")"
...
function generateOrg3Definition() {
  ...
  export FABRIC_CFG_PATH=$PWD
  set -x
  configtxgen -printOrg Org3MSP > ../organizations/peerOrganizations/org3.leadersofdigitalsvo.ru/org3.json
  res=$?
  { set +x; } 2>/dev/null
  if [ $res -ne 0 ]; then
...
function addOrg3 () {
  # If the amigos network is not up, abort
  if [ ! -d ../organizations/ordererOrganizations ]; then
    fatalln "ERROR: Please, run ./network.sh up createChannel first."
  fi

  # generate artifacts if they don't exist
  if [ ! -d "../organizations/peerOrganizations/org3.leadersofdigitalsvo.ru" ]; then
    generateOrg3
